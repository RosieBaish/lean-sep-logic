...
theorem de_morgan'' (A B : Prop) : (A ∧ B) = False ↔ ¬A ∨ ¬B := by {
  cases Classical.em A with
  | inl a  => cases Classical.em B with
              | inl b  => simp [ a,  b];
              | inr nb => simp [ a, nb];
...
noncomputable def singleton_partial_some {A B : Type} (a : A) (b : Option B) : A →. B := match b with
...
@[simp] theorem partial_of_singleton {A B : Type} (a : A) (b : B) (p : A →. B) : ((singleton_partial a b) ⊆ p) ↔ (p a = some b) := by {
  ...
  case mpr => {
    ...
    case left => {
      ...
      rw[← PFun.Dom];
      intro x;
      apply Or.elim (Classical.em (x = a))
      case left => {
        intro temp;
        simp[temp];
        rw [p_a];
...
theorem union_disjoint_symm {p1 p2 : A →. B} : p1 ⊥ p2 → p1 ∪ p2 = p2 ∪ p1 := by {
  ...
  apply funext;
  intro x;
  apply Or.elim (Classical.em (p1 x = Part.none));
  case h.left => {
    intro not_p1_x;
    rw [not_p1_x];
    simp;
    apply Or.elim (Classical.em (p2 x = Part.none));
    case left => {
      intro temp;
      rw[temp];
      simp;
  ...
  case h.right => {
    ...
    case right => {
      ...
      intro temp2;
      rw [← ne_eq] at temp2;
      rw [Part.ne_none_iff] at temp2;
      simp at temp2;
      have ⟨ temp3, temp4 ⟩  := temp2;
      rw [← test_lemma] at temp;
      rw [← ne_eq] at temp;
      rw [Part.ne_none_iff] at temp;
      simp at temp;
      have ⟨ temp5 , temp6 ⟩ := temp;
...
theorem partial_of_p1_union {p1 p2 : A →. B} :  p1 ⊥ p2 → p = p1 ∪ p2 → p1 ⊆ p := by {
  ...
  case right => {
    ...
    intro x1_in_p1;
    simp;
    rw[← Part.eq_some_iff] at x1_in_p1;
    rw [x1_in_p1];
    simp;
  }
}
...
theorem partial_of_difference_self {A B : Type} (p1 p2 : A →. B) : partial_difference p1 p2 ⊆ p1 := by {
  ...
  case left => {
    ...
    case left => {
      ...
      simp [p2x_none];
      intro p1x_dom;
      rw [Part.dom_iff_mem] at p1x_dom;
      exact p1x_dom;
    }
    case right => {
      intro ⟨y, p2x_y⟩;
...
theorem uniqueness {v : Type _} {q : Asrt q} {s : Store v} {h_tilde : Heap v}:
  ...
  | sep q1 q2 => {
    simp[asrt];
    intro ⟨ ⟨ a1 , a2, _  ⟩, b, c ⟩ h h' ⟨ ⟨ h1 , h2 , q1h1 , q2h2 , _ , h_h1_h2 ⟩ , ⟨ h1' , h2' , q1h1' , q2h2' , _ , h_h1_h2' ⟩ ⟩;
...
theorem no_false_neg : (asrt q s h) → (check q s h).1 := by {
  ...
  | emp => simp[asrt, check];
  | singleton v1 v2 => simp[asrt, check]; intro ⟨ a, _ ⟩; exact a;
  | sep q1 q2 => {
    simp[asrt, check];
